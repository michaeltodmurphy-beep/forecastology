...
def hedge_policy(
    ...
    - count=0              ΓåÆ initial_qty                  (initial buy)
    - count=1              ΓåÆ initial_qty * 2              (first recovery)
    - count=factor-1       ΓåÆ initial_qty * 2^(factor-1)  (last allowed buy)
    - count >= factor      ΓåÆ not allowed

    Example: initial=3, factor=3 ΓåÆ allowed counts 0,1,2 ΓåÆ sizes 3,6,12; max=12.
...
class TemperatureStrategy:
    ...
    def __init__(
        ...
        # Idempotency: (ticker, "HH:MM") ΓåÆ local date already evaluated.
        self._intraday_checkpoint_eval_dates: dict[tuple[str, str], datetime.date] = {}
        # Confirmation read: (ticker, "HH:MM") ΓåÆ monotonic time of first below-threshold read.
        # Once set, a second read ΓëÑ60 s later that is still below threshold triggers exit.
        self._intraday_checkpoint_pending: dict[tuple[str, str], float] = {}
        # HWM arm state: (ticker, local_date) ΓåÆ True when armed.
    ...
    async def _run_panic_flatten_exit(
        ...
        *,
        ...
        On trigger, submits a sell at ``sl_panic_sell_price`` (default 1┬ó) so
        ...
        (logged as ``sl.panic_revalidation_degraded``) rather than aborting ΓÇö
        ...
        for attempt in range(1, max_retries + 1):
            ...
            # rather than aborting ΓÇö the initial trigger was already validated
    ...
    async def _seed_rest_price_for_ticker(self, ticker: str) -> bool:
        ...
        if self.cache.get_quote(ticker) is not None or self.cache.get_last_price(ticker) is not None:
            # Already have a price ΓÇö nothing to do.
            return True
        try:
    ...
    async def start(self):
        ...
        # Subscribe to ALL markets via WebSocket ΓÇö no ticker filter so we get
        ...
        # entry scanning ΓÇö decoupled so a slow watchlist cycle never delays SL checks).
        ...
        if self.config.trading_mode == "LIVE":
            try:
                ...
                for ticker, pos_data in api_positions.items():
                    ...
                    if qty <= 0:
                        continue
                    # Skip positions whose market date is before today ΓÇö they have
    ...
    async def _handle_ticker(self, msg: dict):
        ...
        # Cache YES bid/ask from ticker channel ΓÇö this is the authoritative price source
        if yes_bid is not None and yes_ask is not None:
        ...
        # Auto-discover new temperature markets (non-critical path ΓÇö fire-and-forget
    ...
    async def _handle_trade(self, msg: dict):
        ...
        # Update last price synchronously ΓÇö cheap, on the hot path.
    ...
    async def _fetch_live_prices(self, tickers: list[str]) -> dict[str, OrderBook]:
        ...
        Does NOT make REST calls ΓÇö that would be too slow/rate-limited.
        """
        results = {}
        if not tickers:
            return results
    ...
    async def _execute_entry(self, bracket: MarketBracket, ob: Optional[OrderBook] = None, quantity: Optional[int] = None):
        ...
        """
        # ΓöÇΓöÇ NWS gate enforcement ΓÇö final order-submission boundary ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
        ...
        if result.success:
            ...
            async with await self.db.get_session() as session:
                ...
                # the stop_loss_count ΓÇö keeping the counter stuck at 0 and causing
            ...
            # ΓöÇΓöÇ Non-blocking TradeOutcome creation (best-effort) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            asyncio.create_task(
    ...
    async def _partial_fill_chase_loop(
        ...
        if best_bid is None:
            initial_bid = ceiling  # no bid info ΓÇö start at ceiling
        else:
        ...
        try:
            while remaining > 0:
                ...
                # ΓöÇΓöÇ Termination: gate closed ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                ...
                # ΓöÇΓöÇ Termination: falling knife (ask below buy trigger) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                buy_trigger = get_buy_trigger_price(self.config, ticker)
                ob = self.cache.get_orderbook(ticker)
                ...
                # ΓöÇΓöÇ Hard cap guard ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
                hedge_max = int(self.config.hedge_max_factor)
                _, _, max_allowed_qty = hedge_policy(
                    self.config.initial_contract_count, hedge_max, 0
                )
    ...
    async def _create_entry_outcome(
        ...
        try:
            ...
            async with await self.db.get_session() as session:
                # Upsert ΓÇö if a row already exists (e.g. recovery re-entry) update it
        ...
        for ticker, bracket in list(self.active_positions.items()):
            ...
            if current_price is None and not zero_bid_collapse:
                ...
                if (
                    ...
                    # ΓöÇΓöÇ Escalation path for persistently-blind positions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            if stop_loss_reason is not None:
                ...
                # price came directly from the live WS quote cache ΓÇö the watcher
                ...
                # Price has recovered above the stop threshold ΓÇö clear the spread
    ...
    async def _execute_stop_loss(
        ...
        *,
        ...
        """Execute a stop-loss: sell position at market (1┬ó) to guarantee fill.
        ...
        has settled and is gone ΓÇö position is cleaned up quietly as confirmed
        ...
        # means another attempt is in-flight ΓÇö skip this cycle.
        if action_row is None:
            async with await self.db.get_session() as session:
                ...
                try:
                    ...
                    await session.rollback()
                    # A concurrent process beat us to it ΓÇö re-query and check.
            ...
            # FAILED or PENDING ΓÇö transition to SUBMITTED before calling the API.
            async with await self.db.get_session() as session:
                await session.execute(
                    update(OrderAction)
                ...
                await session.commit()
        # action_row is now SUBMITTED ΓÇö proceed to API call.
        ...
        # no longer exists on the exchange.  Treat this as confirmed settlement ΓÇö
        ...
        if bool(remaining["confirmed"]) and remaining_managed_qty == 0:
            ...
            # Partial or unfilled ΓÇö reset action to PENDING so the next cycle retries.
            async with await self.db.get_session() as session:
                await session.execute(
                    update(OrderAction)
                    .where(OrderAction.action_key == action_key)
    ...
    async def _log_snapshot(self):
        ...
        zero sell / exit / order-submission logic ΓÇö it is logging-only.
        """
        now_ts = asyncio.get_event_loop().time()
    ...
    async def _run_low_ticker_closeout(self, now_utc: Optional[datetime.datetime] = None) -> None:
        ...
        the exit path ΓÇö external/manual holdings are not sold.
        """
        low_tickers = [
    ...
    async def _run_intraday_exits(
        ...
        for ticker in low_tickers:
            ...
            # ΓöÇΓöÇ Grace-period check ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            in_grace = False
            if bracket.entry_time is not None:
                age_s = (now_utc - bracket.entry_time).total_seconds()
                if age_s < grace_minutes * 60:
                    in_grace = True
            # Restored positions with no known entry_time ΓåÆ treat as past grace

            # ΓöÇΓöÇ Ask / bid from cache or REST ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
            ...
            if intraday_enabled:
                ...
                for chk_time_str, chk_threshold in schedule:
                    ...
                    if yes_ask is None:
                        # No ask yet ΓÇö skip; retry on next cycle
                        continue
                    ...
                    if elapsed < confirm_seconds:
                        # Still within confirmation window ΓÇö wait
                        continue
            ...
            if hwm_enabled and not exited_this_cycle:
                ...
                if self._hwm_armed.get(hwm_key):
                    ...
                    if below_exit and not in_grace:
                        ...
                        # Ask recovered above exit price ΓÇö clear pending confirmation
